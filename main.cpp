...
void AI_processing(Player players[], on_table_deck on_deck[], int cur_player, int player_num, int state, stack<int>& main_deck, int &state_code, bool &end_game, bool is_AI[]) {
	...
	if (state == 1) {
		...
		if (take_expectation >= 3) {
			...
			cout << "Player " << cur_player + 1 << (is_AI[cur_player] ? "(AI)" : "") << " has taken deck " << tmp + 1 << "." << endl;
			system("PAUSE>nul");
			...
			main_deck.pop();
			state_code = 6;
			cout << "Player " << cur_player + 1 << (is_AI[cur_player] ? "(AI)" : "") << " has drawn ";
			print_card(tmp_card);
			cout << endl << "Player " << cur_player + 1 << (is_AI[cur_player] ? "(AI)" : "") << " has put it to deck " << deck_num + 1<<endl;
			system("PAUSE>nul");
			return;
		}
	}
...
void draw_card(on_table_deck on_deck[], int& state_code, stack<int>& main_deck, int cur_player, int player_num, bool& end_game, Player players[], bool is_AI[]) {
	...
	cout << "Player " << cur_player + 1 << (is_AI[cur_player] ? "(AI)" : "") << " has drawn a ";
	print_card(tmp_card);
	cout << endl << endl;
	for (int i = 0; i<player_num; i++) {
		cout << "Deck" << i + 1 << ": ";
	...
	cout << endl<<endl;
	for (int i = 0; i<player_num; i++) {
		cout << ((players[i].is_over()) ? "[X]" : "[O]") << "Player" << i + 1 << (is_AI[cur_player] ? "(AI)" : "") << ':' << endl;
...
int main()
{
	...
	while (state_code != 0) {
		...
		if (state_code == 3) {//gaming stage start from here
			...
			if (is_all_full) {
				system("CLS");
				cout << "Time for player " << cur_player + 1 << (is_AI[cur_player] ? "(AI)" : "") << endl ;
				for (int i = 0; i < 70; i++) cout << "_";
				cout << endl << endl;
				...
				if (is_AI[cur_player] == false) {
					...
					for (int i = 0; i < 70; i++) cout << "_";
					cout << endl << endl;
					AI_processing(players, on_deck, cur_player, player_num, 1, deck, state_code, end_game, is_AI);
				}
			}
		}
		if (state_code == 4) {//take stage
			if (is_AI[cur_player] == false) taking_cards(on_deck, players, state_code, cur_player, player_num, end_game);
				...
				for (int i = 0; i < 70; i++) cout << "_";
				cout << endl << endl;
				AI_processing(players, on_deck, cur_player, player_num, 2, deck, state_code, end_game, is_AI);
			}
		}
		if (state_code == 5) {//draw stage
			if (is_AI[cur_player] == false) {
				draw_card(on_deck, state_code, deck, cur_player, player_num, end_game, players, is_AI);
			}
			else {
				cout << "Time for player " << cur_player + 1 << (is_AI[cur_player] ? "(AI)" : "") << endl ;
				for (int i = 0; i < 70; i++) cout << "_";
				cout << endl << endl;
				AI_processing(players, on_deck, cur_player, player_num, 3, deck, state_code, end_game, is_AI);
			}
		}
		if (state_code == 6) {
			cur_player++;
			...
			cout << endl;
			system("CLS");
			cout << "State for now:\t" << deck.size() << "cards left" << endl;
			for (int i = 0; i < 70; i++) cout << "_";
			cout << endl<<endl;
			for (int i = 0; i<player_num; i++) {
				cout << "Deck" << i + 1 << ": ";
			...
			for (int i = 0; i<player_num; i++) cout << "Player" << i + 1 << (is_AI[cur_player] ? "(AI)" : "") << ": " << players[i].counting_score() << '\n';
			for (int i = 0; i < 70; i++) cout << "_";
			cout << endl<<endl ;
			for (int i = 0; i<player_num; i++) {
				cout << ((players[i].is_over()) ? "[X]" : "[O]") << "Player" << i + 1 << (is_AI[cur_player] ? "(AI)" : "") << ':' << endl;
		...
		if (state_code == 7) {
			...
			if (end_game) {
				...
				cout << "Cards:" << endl;
				for (int i = 0; i<player_num; i++) {
					cout << "Player" << i + 1 << (is_AI[cur_player] ? "(AI)" : "") << ':' << endl;
					players[i].print_card(player_num);
				}
			}
			system("PAUSE>nul");
		...
		if (state_code == 8) {
			...
			while ((raw_input[0] != 'y' && raw_input[0] != 'Y'&&raw_input[0] != 'n' && raw_input[0] == 'N')||raw_input[1]!=0) {
				cout << "Please enter Y or N to tell the host whether you're willing to restart the game" << endl;
				cout << "Would you like to play another round?" << endl;
				cin >> raw_input;
			}
